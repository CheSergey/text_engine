        ...
        if translation_menu_choice == 1:
            ...
            if translation_error == True:
                ...
                if is_continue:
                    ...
                    chunks_from_json = restore_chunks_from_json(json_data)
                    translated_chunks, translation_error = translator(chunks, settings, client, prompt_text, chunks_from_json, json_data)
...
def translator(chunks, settings, client, prompt_text, chunk_from_json, json_data):
    ...
    start_index = 0 
    if chunk_from_json:
        start_index = len(chunk_from_json)
        json_log = json_data
        translated_chunks = chunk_from_json.copy()
    if settings["debug_mode"] == False:
        for i in range(start_index, len(chunks)):
            ...
            while attempts < 3:
                try:
                    ...
                    json_log += [{"chunk_number":i+1, "translated":translated_chunk, "status":"success", "original_chunk": chunk}]
                    translated_chunks.append(translated_chunk)
                    json_file_creation(json_log)
                    break
                    ...
                    continue
            if attempts == 3:
                json_log += [{"chunk_number":i+1, "translated":"-", "status":"error", "original_chunk":chunk}]
                json_file_creation(json_log)
                translation_error = True
                break
    else:
        is_chunk_error = input("Хотит произвести ошибку на втором чанке? Да\Нет\n\nВаш выбор:").strip()
        if is_chunk_error.lower().startswith("да"):
            for i in range(start_index, len(chunks)):
                chunk = chunks[i]
                if i == 1:
                    print("Тестовая ошибка на втором чанке")
                    json_log += [{"chunk_number":i+1, "translated":"-", "status":"error", "original_chunk":chunk}]
                    json_file_creation(json_log)
                    translation_error = True
                    break
                json_log += [{"chunk_number":i+1, "translated":"-", "status":"debug mode", "original_chunk":chunk}]
                json_file_creation(json_log)
                translated_chunks.append(chunk)
        elif is_chunk_error.lower().startswith("нет"):
            for i in range(start_index, len(chunks)):
                chunk = chunks[i]
                json_log += [{"chunk_number":i+1, "translated":"-", "status":"debug mode", "original_chunk":chunk}]
                json_file_creation(json_log)
                translated_chunks.append(chunk)
    return(translated_chunks, translation_error)

def json_file_creation(json_log):
    with open("backup.json", "w", encoding="utf-8") as file:
        json.dump(json_log, file, ensure_ascii=False, indent=4) # ensure_ascii=False - нормальнный русский текст, indent=4 - красивое форматирование
    
def is_restore_process():
    while True:
        is_continue = input("Хотите продолжить перевод с оборвавшегося чанка? (Да\Нет)\nВаш выбор:").strip()
        if is_continue.lower().startswith("да"):
            return True
        elif is_continue.lower().startswith("нет"):
            return False
        else:
            print("Введите да или нет.")
...
def restore_chunks_from_json(json_data):
    chunks_from_json = []
    for item in json_data:
        if item["status"] == "success":
            try:
                chunks_from_json.append(item["translated"])
            except Exception as e:
                print(e)
    return chunks_from_json
